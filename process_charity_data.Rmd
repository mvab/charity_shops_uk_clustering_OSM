...
```{r}
# test working with OS POI data
all_poi <- readr::read_delim("data/Download_retail_poi_england_rectangle_2046746/poi_4621139/poi-extract-2022_03.csv", delim="|")
...
write.csv(allcharity_w_loc, "data/processed/charity_shops_w_loc.csv")

allcharity_w_loc_OS <- read.csv("data/processed/charity_shops_w_loc.csv")
...
charity<- read.csv("data/processed/charity_shops_w_loc.csv") %>% 
...
write.csv(area_merged, "data/processed/charity_shops_w_loc_w_area_ratings.csv")
area_merged<- read.csv("data/processed/charity_shops_w_loc_w_area_ratings.csv")


#generation test data for areas
area_merged %>% filter(grepl("Bristol", lsoa11nm)) %>% 
  write.csv("data/processed/charity_shops_w_loc_w_area_ratings_Bristol.csv")

area_merged %>% filter(grepl("Leeds", lsoa11nm)) %>% 
  write.csv("data/processed/charity_shops_w_loc_w_area_ratings_Leeds.csv")
...
```{r}
# OSM data
dat <- vroom("data/charity_coords_new.csv") %>% select(index, name, latitude = lat, longitude = lon) %>% mutate(row = row_number())
dat_geo <- data_frame()
...
vroom_write(charityOSM, "data/processed/charity_uk_OSM_coords_w_postcode.csv")
...
write.csv(area_mergedOSM, "data/processed/charity_shopsOSM_w_loc_w_area_ratings.csv")
...
area_mergedOSM  %>% filter(grepl("Bristol", lsoa11nm)) %>% 
  write.csv("data/processed/charity_shopsOSM_w_loc_w_area_ratings_Bristol.csv")

area_mergedOSM  %>% filter(grepl("Leeds", lsoa11nm)) %>% 
  write.csv("data/processed/charity_shopsOSM_w_loc_w_area_ratings_Leeds.csv")

area_mergedOSM  %>% filter(grepl("Southampton", lsoa11nm)) %>% 
  write.csv("data/processed/charity_shopsOSM_w_loc_w_area_ratings_Southampton.csv")

area_mergedOSM  %>% filter(grepl("Brighton and Hove", lsoa11nm)) %>% 
  write.csv("data/processed/charity_shopsOSM_w_loc_w_area_ratings_Brighton.csv")

area_mergedOSM  %>% filter(grepl("Warwick", lsoa11nm)) %>% 
  write.csv("data/processed/charity_shopsOSM_w_loc_w_area_ratings_Warwick.csv")

area_mergedOSM  %>% filter(grepl("Newcastle upon Tyne", lsoa11nm)) %>% 
  write.csv("data/processed/charity_shopsOSM_w_loc_w_area_ratings_Newcastle.csv")

area_mergedOSM  %>% filter(grepl("York", lsoa11nm)) %>% 
  write.csv("data/processed/charity_shopsOSM_w_loc_w_area_ratings_York.csv")

area_mergedOSM  %>% filter(grepl("Cheltenham", lsoa11nm)) %>% 
  write.csv("data/processed/charity_shopsOSM_w_loc_w_area_ratings_Cheltenham.csv")


area_mergedOSM  %>% filter(grepl("Cambridge", lsoa11nm)) %>% 
  write.csv("data/processed/charity_shopsOSM_w_loc_w_area_ratings_Cambridge.csv")
...
os_vs_osm <- full_join(
  area_merged %>% count(ladnm) %>% rename(os_n=n) %>% mutate(ladnm = gsub(", City of", "", ladnm)) %>%  mutate(ladnm = gsub(", County of", "", ladnm)),
  counts_by_ladnn %>% rename(osm_n = total)) %>% 
  replace(is.na(.), 0) %>% 
