...
```{r}
# deprivation data
imd_data <- vroom("data/imd2019lsoa.csv")
dim(imd_data)
...
areas_data<-vroom("data/PCD_OA_LSOA_MSOA_LAD_MAY19_UK_LU.csv") # lsoa11cd is the LSOA
dim(areas_data)
place_names_data <- vroom("data/MSOA-Names-1.17.csv") %>% select(msoa11cd, msoa11nm,msoa11hclnm)
...
postcode_data <- vroom("data/open_postcode_geo.csv/open_postcode_geo.csv", col_names = F, col_select=c(X7, X1, X8,X9))
...
# LSOA centroids
lsoa_coords <- vroom("data/Lower_Layer_Super_Output_Areas_(December_2011)_Population_Weighted_Centroids.csv")
...
```{r}
# house price data
all.files <- list.files(path = "data/price_paid/", pattern = "pricepaid*", full.names = T)
...
house_price_data_sub<- house_price_data %>%
  ...
  filter(price >=38000 & price <= 10000000) %>% # 10M max
  arrange(price) %>% 
...
for (i in 1:length(areas$ladcd)){
  ...
  write.csv(compare_wo_out,paste0("data/processed/figures/removed_", area_name, ".csv" ))
  ...
  # histograms
   hist1 <- ggplot(prices_merged_area, aes(x=price))+ geom_histogram(color="black", fill="white")
    ...
    ggsave(paste0("data/processed/figures/hist_", area_name, ".png"),
       plot=h, scale=1, 
       width=12, height=20,
  ...
  p2 <- ggplot(data = prices_merged_wo_out_by_lsoa, 
   ...
   theme(legend.position = "none",
    ...
    ggsave(paste0("data/processed/figures/boxplot_", area_name, ".png"),
       plot=p, scale=1, 
       width=30, height=25,
       units=c("cm"), dpi=200, limitsize=F)
...
for (area in areas){
  ...
  tidy_lsoa_sub <- tidy_lsoa %>% 
    filter(ladnm == area) %>% 
    # ratio 1: not vety useful - will drop later
